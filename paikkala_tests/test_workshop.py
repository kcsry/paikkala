...
def test_workshop_program(workshop_program) -> None:
    assert workshop_program


@pytest.mark.django_db
def test_missing_contact(workshop_program, workshop_zone, user_client) -> None:
    with pytest.raises(ContactRequired):
        list(workshop_program.reserve(zone=workshop_zone, count=1, user=user_client.user))


@pytest.mark.django_db
def test_with_contact(workshop_program, workshop_zone, workshop_row, user_client) -> None:
...
@pytest.mark.django_db
def test_form(workshop_program, workshop_zone, workshop_row, user_client) -> None:
    assert workshop_program.is_reservable()
