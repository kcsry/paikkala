...
from paikkala.models.tickets import Ticket, generate_key
...
class Row(models.Model):
    zone = models.ForeignKey(
        "paikkala.Zone", related_name="rows", on_delete=models.CASCADE
    )
    ...
    def get_numbers(
        self, additional_excluded_set: Optional[Set[int]] = None
    ) -> List[int]:
    ...
    def reserve(
        ...
        *,
        ...
        reserved_numbers = set(
            program.tickets.filter(row=self).values_list("number", flat=True)
        )
        unreserved_numbers = [
            number
        ...
        if attempt_sequential and count > 1:
            ...
            acceptable_sequential_runs = [
                run for run in sequential_runs if len(run) >= count
            ]
        ...
        for t in tickets:
            t.recompute_qualifiers()

        yield from Ticket.objects.bulk_create(tickets)
